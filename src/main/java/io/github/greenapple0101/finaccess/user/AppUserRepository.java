// [이 파일의 역할: AppUser 저장소 계약]
// interface는 구현 방법이 아니라 사용할 기능의 계약을 표현하는 Java 문법입니다.
// extends JpaRepository<AppUser, UUID>는 기본 저장·조회 메서드를 상속합니다.
// 첫 타입 인자 AppUser는 엔티티, 두 번째 UUID는 식별자 타입입니다.
...
public interface AppUserRepository extends JpaRepository<AppUser, UUID> {
    // 메서드 이름에서 조건을 유도하는 Spring Data 기능입니다.
    // findBy 뒤의 IdentityIssuer와 IdentitySubject는 엔티티의 필드명이고 And는 두 조건을 모두 요구합니다.
    // 개념상 WHERE identity_issuer = ? AND identity_subject = ? 로 조회합니다.
    // 문자열을 SQL에 직접 이어 붙이는 코드가 아니라 값 매개변수로 바인딩합니다.
    // Optional<AppUser>는 값이 없을 가능성을 호출자에게 알려줍니다.
